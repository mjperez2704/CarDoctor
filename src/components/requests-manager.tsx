// src/components/requests-manager.tsx
"use client";
...
// Importamos el nuevo tipo de dato desde las acciones
import type { InternalRequest } from "@/app/(protected)/internal/requests/actions";
import { Badge } from "./ui/badge";

type RequestsManagerProps = {
    initialRequests: InternalRequest[];
    employees: Empleado[];
};
...
export function RequestsManager({ initialRequests, employees }: RequestsManagerProps) {
    ...
    // Esta función ya no es necesaria, el nombre viene en la consulta
    // const getEmployeeName = (employeeId: number) => { ... };

    const statusVariant: Record<InternalRequest["estado"], "default" | "secondary" | "destructive" | "outline"> = {
        PENDIENTE: "secondary",
        APROBADA: "default",
    ...
    const typeVariant: Record<InternalRequest["tipo"], "default" | "secondary" | "destructive" | "outline"> = {
        COMPRA: "outline",
        GASTO: "outline",
        PERMISO: "secondary",
                    ...
                    <TableBody>
                        {requests.map((request) => (
                            <TableRow key={request.id}>
                                ...
                                <TableCell>{request.solicitante_nombre}</TableCell>
                                <TableCell>
                                ...
                                <TableCell className="text-right">
                                    {request.monto ? `$${Number(request.monto.toFixed(2))}` : "-"}
                                </TableCell>
                                <TableCell>
                                    <DropdownMenu>
